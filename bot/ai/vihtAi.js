...
function cannedResponse(prompt) {
  ...
  const nameSandraRx = /(?<!\p{L})(?:сандра|sandra|sandra\s+goslin|sandra\s+viht|девушка\s+андрея)(?!\p{L})/iu;
  const nameNayaRx = /(?<!\p{L})(?:naya\s+bay|naya|noya|ней\s+бей|ной\s+бой|ная)(?!\p{L})/iu;

  // === ABOUT VIHT VPN PROJECT ===
  
  // Любой вопрос про Viht VPN (когда создан, когда начали, когда сделан и т.д.)
  if (/(?:viht|vpn|vihtai)/iu.test(p) && /(?:когда|сделан|создан|начал|разработ|релиз|запуск|начало|версия|дата)/iu.test(p)) {
    return `🕐 **История Viht VPN:**\n— Разработка началась: **начало 2025 года**\n— Официальный релиз: **20.05.2025**\n\nЭто был долгий путь создания современного и надежного VPN сервиса. Узнать подробнее: https://vihtai.pro`;
  }

  // Сайт Viht
  if (/(?:ссылка\s+на\s+сайт|дай\s+ссылку|сайт\s+viht|где\s+сайт)/iu.test(p) && /(?:viht|vihtai)/iu.test(p)) {
    return `🌐 **Официальный сайт Viht:**\nhttps://vihtai.pro`;
  }

  // === ABOUT DISCORD SERVER ===
  
  // Когда был создан ДС сервер
  if (/(?:когда\s+(?:создан|создана)|дата\s+создания|когда\s+открыл|когда\s+начал).*(?:дс|discord|сервер)/iu.test(p) || /(?:дс|discord|сервер).*(?:дата|когда|создан)/iu.test(p)) {
    return `📅 **Дата создания Discord сервера Viht:**\n**13 августа 2025 года**\n\nОфициальный сервер для общения и информации о проекте Viht.`;
  }

  // Ссылка на ДС сервер / официальный сервер Андрея
  if (/(?:есть\s+ли|официальный|какой|ссылка\s+на).*(?:дс|discord|сервер)/iu.test(p) && /(?:viht|андрей|official)/iu.test(p)) {
    return `💬 **Официальный Discord сервер Viht:**\nhttps://discord.gg/7TYB5bj79t\n\nЭто сервер для общения, поддержки и информации о проекте Viht. Присоединяйся к нам!`;
  }

  // Информация о ДС сервере / что там
  if (/(?:расскажи\s+про|что\s+это|что\s+там|какой).*(?:дс|discord|сервер)/iu.test(p) && /viht/iu.test(p)) {
    return `🎮 **Viht ОФИЦАЛ Discord сервер:**\nЭто сервер для общения и информации о проекте Viht.\n\n📍 Адрес: https://discord.gg/7TYB5bj79t\n📅 Создан: 13 августа 2025 года\n\nЗдесь ты можешь получить поддержку, узнать последние новости о VPN Viht и общаться с сообществом.`;
  }
  ...
  if (whoRx.test(p) && nameAndreyRx.test(p)) {
    return `👨‍💻 **Андрей Вихт** — создатель и главный разработчик системы VPN Viht, основатель компании Viht.\n\n💫 **О нём:** Грамотный, умный и очень хороший человек, который вложил всю душу в развитие проекта.\n\n💑 **Рядом:** Поддержку и вдохновение он получает от **Сандры** — своей любимой девушки, которая помогает команде.\n\n🔗 Узнать больше: https://vihtai.pro\n🎮 Официальный сервер: https://discord.gg/7TYB5bj79t`;
  }

  // SANDRA - only if user explicitly asks "who is"
  if (whoRx.test(p) && nameSandraRx.test(p)) {
    return `💖 **Sandra (Сандра)** — девушка создателя **Андрея Вихта**.\n\n✨ **О ней:** Помощник и самый любимый человек в проекте Viht. Очень тёплый, заботливый и вдохновляющий человек. Она активно поддерживает команду и помогает сообществу.\n\n💑 Вместе с Андреем они создают проект мечты.`;
  }

  // NAYA - only if user explicitly asks "who is"
  if (whoRx.test(p) && nameNayaRx.test(p)) {
    return `🎭 **Naya (Naya Bay)** — член команды Viht.\n\n⭐ **О ней:** Прекрасный человек, который является сердцем команды. Всегда смешит, веселит и поддерживает коллектив. Несёт за собой юмор, позитив и стремление помогать.`;
  }

  // === ABOUT BOT ===

  // Официальный ли бот
  if (/(?:официальный\s+ли|есть\s+ли.*официальный|как\s+(?:он|это)\s+называется).*(?:бот|ai)/iu.test(p)) {
    return `🤖 **Я — официальный бот проекта Viht!**\n\nСоздан специально для сервера **Viht ОФИЦАЛ**: https://discord.gg/7TYB5bj79t\n\nЯ помогаю с информацией о VPN Viht, отвечаю на вопросы и поддерживаю сообщество.`;
  }
  ...
  // DOWNLOADS - only respond when user explicitly mentions Viht/VPN
  if (/(?:viht|vpn|vihtai)/iu.test(p) && /(?:скач|download|install|установ|загруз|ссылка|где|как|получить)/iu.test(p) && /(?:приложен|app|android|ios|windows|виндовс|скач)/iu.test(p)) {
  ...
  // KEY/AUTH - require explicit mention of Viht/VPN before revealing details
  if (/(?:viht|vpn|vihtai)/iu.test(p) && /\b(ключ|создать\s+ключ|create\s+key|auth|авторизоваться|авторизация)\b/i.test(p)) {
    return `🔑 **Как создать ключ:**\n1. Перейди на https://vihtai.pro\n2. Авторизуйся через Telegram\n3. Выбери подходящее устройство (Android, iOS, Windows)\n4. Создай ключ доступа\n5. Скачай и установи приложение на нужную платформу\n\nГотово! Теперь можешь подключаться к VPN.`;
  }
...
async function sendPrompt(prompt, opts = {}) {
  ...
  const payload = {
    contents: contents,
    systemInstruction: {
      parts: [{ text: `Ты — виртуальный помощник проекта Viht. Не инициируй разговор о Viht, VPN, сервисах или ссылках проекта, пока пользователь прямо не спросит об этом. Не раскрывай своё имя или модель, пока пользователь прямо не спросит. Если пользователь прямо спрашивает о Viht, VPN или службах — отвечай корректно и по существу, включая инструкции и ссылки при необходимости.
